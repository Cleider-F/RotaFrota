...
Na tela de login, Criar conta recebe e-mail, senha (6–128 caracteres) e confirmação. rotafrotaRegisterAccount verifica a autorização ativa antes de criar a conta. Após criar a conta, o aplicativo faz login e chama rotafrotaActivateAccount: um e-mail ainda autorizado e vinculado à empresa correta ganha o documento de membro e as permissões correspondentes. Não há envio ou exigência de confirmação de e-mail. O cadastro não aceita o perfil escolhido pelo usuário. Senhas nunca são registradas em documentos ou logs pela aplicação. Contas já existentes usam Entrar ou Esqueci minha senha; nunca redefinimos sua senha ao autorizar.
...
O acesso é liberado imediatamente após cadastro e login, mediante nova verificação da autorização no servidor. Desativar preserva conta Firebase, viagens e auditoria, bloqueando acesso futuro ao RotaFrota. Não remove arquivos já baixados ou dados já carregados. Lista limitada a 200 entradas com aviso. Novo cadastro está indisponível no modo demo.

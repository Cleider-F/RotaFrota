  ...
  return <div className="bi-panel">
    <section className="bi-filters" aria-label="Filtros do BI"><div className="bi-filter-heading"><b><SlidersHorizontal size={18}/> Analisar operação</b><button className="secondary" onClick={()=>update({from:'',to:'',plate:'',driver:'',status:'all',day:''})}>Limpar filtros</button></div>
      <div className="bi-filter-fields">
        ...
        <label>Situação<select value={filter.status} onChange={e=>update({status:e.target.value})}><option value="all">Todas</option><option value="active">Em andamento</option><option value="finished">Concluídas</option><option value="cancelled">Canceladas</option></select></label>
      </div>
      <p>Período pela data de início da viagem, no horário de Brasília. Os registros recebidos atualizam esta análise. Cancelamentos preservam quilômetros e litros já registrados nos totais, mas não entram no km/L.</p>
    ...
    <div className="bi-chart-grid">
      ...
      <section className="bi-card"><h2>Situação das viagens</h2><p>Clique em uma situação para filtrar o painel.</p><div className="bi-status">
        ...
        <button aria-pressed={filter.status==="cancelled"} onClick={()=>update({status:filter.status==="cancelled"?"all":"cancelled"})}><span className="bi-status-dot cancelled"/><span>Canceladas</span><strong>{summary.cancelled}</strong></button><div className="bi-status-bar" style={{background:"#b97168"}} aria-label={`${summary.active} em andamento e ${summary.finished} concluídas e ${summary.cancelled} canceladas`}><span style={{width:`${summary.count ? 100*summary.active/summary.count : 0}%`}}/><span style={{width:`${summary.count ? 100*summary.finished/summary.count : 0}%`,background:"#325e48"}}/></div>
    ...
    <section className="bi-card"><div className="bi-compare-heading"><div><h2>Viagens deste recorte</h2><p>Abra uma viagem para conferir registros, fotos e correções. O CSV operacional não inclui viagens canceladas.</p></div><button className="secondary" disabled={!scoped.length} onClick={exportSelection}><Download size={17}/> Exportar recorte</button></div>
      {!scoped.length ? <p className="bi-empty">Nenhuma viagem encontrada. Ajuste os filtros.</p> : <div className="bi-table-scroll"><table><thead><tr><th>Início</th><th>Motorista / veículo</th><th>Nota</th><th>Distância</th><th>Litros</th><th>Situação</th><th>Detalhes</th></tr></thead><tbody>{scoped.slice(0,limit).map(t=><tr key={t.id}><td>{date(t.startedAt)}</td><td><b>{t.driver}</b><br/>{t.plate}</td><td>{t.invoice}</td><td>{number(distance(t))} km</td><td>{number(totalLiters(t),2)} L</td><td>{t.cancelledAt?'Cancelada':t.endedAt?'Concluída':'Em andamento'}</td><td><button className="secondary" aria-label={`Abrir viagem de ${t.driver}, nota ${t.invoice}`} onClick={()=>onOpenTrip(t)}>Abrir viagem</button></td></tr>)}</tbody></table></div>}
